...
class Ctl_nodecatagory extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        // setting
        $this->model = $this->mdl_nodecatagory;
    }

    public function index()
    {
        $data['catagory'] = $this->model->get_dataShow();

        $this->template->set_layout('lay_datatable');
        $this->template->title('ข้อมูลประเภทผู้ติดต่อ');
        $this->template->build('nodecatagory/index',$data);
    }
    ...
    public function get_data()
    {
        ...
        if ($data) {
            foreach ($data as $row) {
                ...
                $sub_data['CREATER'] = whois('id', $row->USER_STARTS);
                $sub_data['DATE_STARTS'] = array(
                    "display"   => toThaiDateTimeString($row->DATE_STARTS, 'datetime'),
