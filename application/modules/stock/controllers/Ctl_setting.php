...
class Ctl_setting extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('mdl_stock_limit');

        $this->middleware();

        $this->model = $this->mdl_stock_limit;
    }

    public function index()
    {
        /* $this->template->set_layout('lay_datatable');
        $this->template->title('max/min');
        $this->template->build('sitting/index'); */
    }

    public function limit()
    {
        $data['item'] = $this->model->get_dataShow_noneset();
        
        $this->template->set_layout('lay_datatable');
        $this->template->title('ตั้งค่า max/min');
        $this->template->build('limit/index',$data);
    }
